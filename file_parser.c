...
/*
 * This function decoded the APEX CPU Commands.
 *
 * Note: You are free to edit this function according to your implementation
 */

CPU_COMMAND *
process_cpu_commands(char const *commands[])
{
    if(!commands[0])
    {
        return NULL;
    }
    CPU_COMMAND *command = malloc(sizeof(CPU_COMMAND));

    if (strcmp(commands[0], "simulate") == 0)
    {
        if(!commands[1])
        {
            return NULL;
        ...
        command->data = atoi(commands[1]);
    }
    else if (strcmp(commands[0], "display") == 0)
    {
        if(!commands[1])
        {
            return NULL;
    ...
    else if (strcmp(commands[0], "single_step") == 0)
    {
        command->cmd = SINGLE_STEP;
    }
    else if (strcmp(commands[0], "show_mem") == 0)
    {
        if(!commands[1])
        {
            return NULL;
        }
        command->cmd = SHOW_MEM;
        command->data = atoi(commands[1]);
    }
    else 
    {
